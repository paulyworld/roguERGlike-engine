...
func configure(
	...
	max_hr: int,
	target_wkg: float,
	recovery_ceiling_wkg: float,
	hr_zone_bounds: Array[int]
) -> void:
	_weight_kg = weight_kg
	_max_hr = max_hr
	_target_wkg = target_wkg
	_recovery_ceiling_wkg = recovery_ceiling_wkg
...
func _power_max() -> float:
	var max_power: float = max(_target_wkg * _weight_kg * 1.35, 400.0)
...
func _draw_targets(plot: Rect2) -> void:
	...
	var target_y: float = _y_for_value(plot, _target_wkg * _weight_kg, power_max)
	var recovery_y: float = _y_for_value(plot, _recovery_ceiling_wkg * _weight_kg, power_max)
	draw_line(Vector2(plot.position.x, target_y), Vector2(plot.end.x, target_y), Color(1.0, 0.7, 0.2), 2.0)
	draw_line(Vector2(plot.position.x, recovery_y), Vector2(plot.end.x, recovery_y), Color(0.35, 1.0, 0.45), 1.5)
...
func _draw_legend(plot: Rect2) -> void:
	...
	font.draw_string(get_canvas_item(), Vector2(x + 168.0, y), "targets", HORIZONTAL_ALIGNMENT_LEFT, -1.0, font_size, Color(1.0, 0.75, 0.25))
